...
if g:aurepl_elm_started == 0
  let g:aurepl_elm_output_file = strftime('%d%m%y%H%M%S')
  call jobstart('(while [ 1 ]; do sleep 1; done) |  elm-repl 2>&1 | tee -a ' . g:aurepl_elm_output_file . '.elm &')
...
autocmd BufEnter * if !exists('b:aurepl_use_command') && s:should_bind() | let b:aurepl_use_command = 'echo ; outputfile=' . g:aurepl_elm_output_file . ';pid=' . g:aurepl_elm_pid . '; echo $(cat ./scratch.temp.elm) > /proc/$pid/fd/0 && sleep 1 && (while [ `tail -n 1 ./$outputfile.elm | grep -o "\s:\s*"` ]; do sleep 1; done) && tail -n 2 ./$outputfile.elm | head -n 1' | endif
